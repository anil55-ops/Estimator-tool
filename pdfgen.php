...
$result_check = $wpdb->query($wpdb->prepare(
    "INSERT INTO $table_name (fullname, email, phone, file_name, cost_as_per_sqt, cost_per_ceiling_height, cost_as_per_bathroom_preference, bathroom_drain_cost, cost_as_per_kitchenette_preference, kitchenette_drain_cost, ceiling_type_cost, total_cost) 
...
function send_custom_email($attachment, $admin_email)
{
    ...
    // Send the email
    $result_check = wp_mail($email, $subject, $body, $headers, $attachment);
...
function send_custom_email_admin($attachment, $admin_email)
{
    ...
    // Send the email
    $result_check = wp_mail($to, $subject, $body, $headers, $attachment);
